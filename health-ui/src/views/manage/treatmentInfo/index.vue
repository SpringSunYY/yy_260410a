    ...
    <el-table v-loading="loading" :data="treatmentInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="编号" align="center" prop="id" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="就诊时间" align="center" prop="visitTime" width="180" v-if="columns[2].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.visitTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="诊断结果" align="center" prop="diagnosis" v-if="columns[3].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="用药处方" align="center" prop="prescription" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="附件" align="center" prop="appendix" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="所属用户" align="center" prop="userId" width="100" v-if="columns[6].visible">
                <template #default="scope">
                  <file-view :file-url="scope.row.userId"/>
                </template>
              </el-table-column>
                <el-table-column label="创建人" align="center" prop="createBy" v-if="columns[7].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="更新人" align="center" prop="updateBy" v-if="columns[9].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['manage:treatmentInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['manage:treatmentInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="treatmentInfoRef" :model="form" :rules="rules" label-width="80px">
                        ...
                        <el-form-item label="所属用户" prop="userId">
                          <file-upload v-model="form.userId"/>
                        </el-form-item>
...
<script setup name="TreatmentInfo">
  import { listTreatmentInfo, getTreatmentInfo, delTreatmentInfo, addTreatmentInfo, updateTreatmentInfo } from "@/api/manage/treatmentInfo";
