    ...
    <el-table v-loading="loading" :data="hospitalizationInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="编号" align="center" prop="id" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="入院时间" align="center" prop="inTime" width="180" v-if="columns[2].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.inTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="出院时间" align="center" prop="outTime" width="180" v-if="columns[3].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.outTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="住院机构" align="center" prop="hospitalName" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="住院原因" align="center" prop="hospitalReason" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="所属用户" align="center" prop="userId" v-if="columns[8].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['manage:hospitalizationInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['manage:hospitalizationInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="HospitalizationInfo">
  import { listHospitalizationInfo, getHospitalizationInfo, delHospitalizationInfo, addHospitalizationInfo, updateHospitalizationInfo } from "@/api/manage/hospitalizationInfo";
