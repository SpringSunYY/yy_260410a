<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="remindConfigInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="提醒标题" align="center" prop="remindTitle" v-if="columns[3].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="提醒内容" align="center" prop="remindContent" v-if="columns[4].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="提醒时间" align="center" prop="remindTime" width="180" v-if="columns[5].visible"
                       :show-overflow-tooltip="true">
        <template #default="scope">
      ...
      <el-table-column label="每天提醒次数" align="center" prop="remindFrequency" v-if="columns[8].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="提醒间隔分钟" align="center" prop="remindInterval" v-if="columns[9].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="上次提醒时间" align="center" prop="lastRemindTime" width="180" v-if="columns[10].visible"
                       :show-overflow-tooltip="true">
        <template #default="scope">
          <span>{{ parseTime(scope.row.lastRemindTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="下次提醒时间" align="center" prop="nextRemindTime" width="180" v-if="columns[11].visible"
                       :show-overflow-tooltip="true">
        <template #default="scope">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['manage:remindConfigInfo:edit']">修改
          </el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['manage:remindConfigInfo:remove']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="600px" append-to-body>
      <el-form ref="remindConfigInfoRef" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="居民" prop="residentId">
          <el-select
        ...
        <el-form-item label="每天提醒次数" prop="remindFrequency">
          <el-input-number :min="1" style="width: 100%;" v-model="form.remindFrequency"
                           placeholder="请输入每天提醒次数"/>
        </el-form-item>
        <el-form-item label="提醒间隔分钟" prop="remindInterval">
          <el-input-number :min="1" style="width: 100%;" v-model="form.remindInterval"
                           placeholder="请输入提醒间隔分钟"/>
        </el-form-item>
...
import {
  listRemindConfigInfo,
  getRemindConfigInfo,
  delRemindConfigInfo,
  addRemindConfigInfo,
  updateRemindConfigInfo
} from "@/api/manage/remindConfigInfo";
...
const data = reactive({
  ...
  columns: [
    ...
    {key: 4, label: '提醒内容', visible: false},
    {key: 5, label: '提醒时间', visible: false},
    {key: 6, label: '开始时间', visible: true},
    {key: 7, label: '结束时间', visible: true},
    {key: 8, label: '每天提醒次数', visible: false},
    {key: 9, label: '提醒间隔分钟', visible: false},
    {key: 10, label: '上次提醒时间', visible: true},
    {key: 11, label: '下次提醒时间', visible: true},
    {key: 12, label: '所属用户', visible: false},
