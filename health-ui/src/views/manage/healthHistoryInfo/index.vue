    ...
    <el-table v-loading="loading" :data="healthHistoryInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="编号" align="center" prop="id" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="测量时间" align="center" prop="measureTime" width="180" v-if="columns[2].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.measureTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="身高" align="center" prop="height" v-if="columns[3].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="体重" align="center" prop="weight" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="收缩压mmHg" align="center" prop="sbp" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="舒张压mmHg" align="center" prop="dbp" v-if="columns[6].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="血糖mmol/L" align="center" prop="bloodGlucose" v-if="columns[7].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="心率次/分" align="center" prop="heartRate" v-if="columns[8].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="所属用户" align="center" prop="userId" v-if="columns[11].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['manage:healthHistoryInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['manage:healthHistoryInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="healthHistoryInfoRef" :model="form" :rules="rules" label-width="80px">
                        ...
                        <el-form-item label="收缩压mmHg" prop="sbp">
                          <el-input v-model="form.sbp" placeholder="请输入收缩压mmHg" />
                        </el-form-item>
                        <el-form-item label="舒张压mmHg" prop="dbp">
                          <el-input v-model="form.dbp" placeholder="请输入舒张压mmHg" />
                        </el-form-item>
                        <el-form-item label="血糖mmol/L" prop="bloodGlucose">
                          <el-input v-model="form.bloodGlucose" placeholder="请输入血糖mmol/L" />
                        </el-form-item>
                        <el-form-item label="心率次/分" prop="heartRate">
                          <el-input v-model="form.heartRate" placeholder="请输入心率次/分" />
                        </el-form-item>
                        <el-form-item label="测量类型" prop="measureType">
...
<script setup name="HealthHistoryInfo">
  import { listHealthHistoryInfo, getHealthHistoryInfo, delHealthHistoryInfo, addHealthHistoryInfo, updateHealthHistoryInfo } from "@/api/manage/healthHistoryInfo";
